...
app.put("/coffee/status/:status/:id", (req, res) => {
  ...
  fs.readFile(coffeesFilePath, "utf8", (err, data) => {
    ...
    if (orders.length > 0) {
      ...
      if (orderIndex !== -1) {
        ...
        fs.writeFile(coffeesFilePath, JSON.stringify(orders), (err) => {
          if (err) {
            console.log(err);
            res.status(500).send("Cannot write to the file");
            return;
          }

          res.status(202).send("Order status updated successfully");
        });
      } else {
...
app.post("/coffee/:id/comps/:compId/value/:value", (req, res) => {
  ...
  fs.readFile(path.join(__dirname, coffeesFilePath), "utf8", (err, data) => {
    ...
    const component = product.components.find(
      (component) => component[0] === componentId
    );
...
app.put("/orders/status/:status/:id", (req, res) => {
  const status = req.params.status;
  const id = req.params.id;
  const filePath = path.join(ordersFilePath, `${id}.json`);
  ...
  fs.readFile(filePath, "utf8", (err, data) => {
    ...
    let order = JSON.parse(data); // Módosítás: Egyetlen objektumként tároljuk az order változóban

    if (order) {
      order.status = status;
      fs.writeFile(filePath, JSON.stringify(order), (err) => {
        // Módosítás: Csak az order objektumot írjuk vissza
        if (err) {
          console.log(err);
          res.status(500).send("Cannot write to the file");
          return;
